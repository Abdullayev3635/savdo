...
import '../widgets/korzina_items.dart';
  ...
  class ClientKorzinaScreen extends StatefulWidget {
  const ClientKorzinaScreen({Key? key}) : super(key: key);

  static Widget screen() => BlocProvider(
        create: (context) => di<KorzinaBloc>()..add(KorzinaInitialEvent()),
        child: const ClientKorzinaScreen(),
      );

  @override
  State<ClientKorzinaScreen> createState() => _ClientKorzinaScreenState();
}

class _ClientKorzinaScreenState extends State<ClientKorzinaScreen> {
  num jamiSumma = 0;
    ...
    return Scaffold(
      body: BlocBuilder<KorzinaBloc, KorzinaState>(
        builder: (context, state) {
          ...
          return ValueListenableBuilder<Box<ClientKorzinaCard>>(
              valueListenable: Hive.box<ClientKorzinaCard>(korzinaBox).listenable(),
              builder: (context, box, _) {
                jamiSumma = 0;
                var transaction = box.values.toList().cast<ClientKorzinaCard>();
