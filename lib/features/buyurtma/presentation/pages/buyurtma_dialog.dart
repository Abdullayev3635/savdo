...
class _BuyurtmaDialogState extends State<BuyurtmaDialog> {
  ...
  void initState() {
    ...
    dialogBloc.add(BuyurtmaInitialLocalEvent());
  }
            ...
            BlocBuilder<BuyurtmaDialogBloc, BuyurtmaDialogState>(
              builder: (context, state) {
                if (state is BuyurtmaDialogSelectedSuccessState) {
                  List<CurrencyModel>? currencyList = state.buyurtmaList[0].currency;
                  List<PriceTypeModel>? priceTypeList = state.buyurtmaList[0].priceType;
                  List<StoreModel>? storeList = state.buyurtmaList[0].stores;
                  List<BranchModel>? branchList = state.buyurtmaList[0].branchs;
                  if (isFirstTap) {
                                      ...
                                      child: ListView.builder(
                                        ...
                                        itemBuilder: (context, index) {
                                          return state.debitList[index].value! < 0
                                              ? Text(
                                            "${formatter.format(double.parse(state.debitList[index].value.toString().replaceAll(",", "")))} ${state.debitList[index].name}",
                                            maxLines: 1,
                          ...
                          if (clientName != 'Mijozni tanlang') {
                            ...
                            sharedPreferences.setString(
                                ...
                                currencyList[int.parse(narxTuriGroup)]
                                    .name
                                    .toString());
                            sharedPreferences.setString(sharedStoreId, storeList![int.parse(storeGroup)].id.toString());
                            // if(sharedPreferences.getBool(sharedIsAgent) == true) {
                            //   sharedPreferences.setString(
                            //       sharedBuyurtmaBranchId,
                            //       branchList![int.parse(branchGroup)].id
                            //           .toString());
                            // }
                            sharedPreferences.setString(
                                sharedPriceTypeId,
