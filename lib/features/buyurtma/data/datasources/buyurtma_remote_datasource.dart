...
class BuyurtmaRemoteDataSourceImpl implements BuyurtmaRemoteDataSource {
  ...
  Future<dynamic> getBuyurtma(int workerId) async {
    ...
    String branch_id = sharedPreferences.getString(sharedBuyurtmaBranchId) ??
        (sharedPreferences.getString(sharedBranchId) ?? "0");

    List<BuyurtmaModel> buyurtmaList = [];
    dynamic json = {"worker_id": workerId, "branch_id": branch_id};
    try {
      ...
      if (response.statusCode == 200) {
        try {
          ...
          for (int i = 0; i < (parsed["branchs"] as List).length; i++) {
            branchs.add(BranchModel.fromJson(parsed["branchs"][i]));
          }
