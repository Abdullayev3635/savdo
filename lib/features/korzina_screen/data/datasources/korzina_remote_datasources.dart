...
class KorzinaOrderRemoteDatasourceImpl extends KorzinaOrderRemoteDatasource {
  ...
  Future<List<ErrorModel>> sendKorzinaData(
    ...
    try {
      ...
      bool isAgent = sharedPreferences.getBool(sharedIsAgent) ?? false;
      ...
      dynamic json1 = {
        ...
        'branch_id': int.parse(sharedPreferences.getString(sharedBuyurtmaBranchId) ?? "0"),
        "suplier_id": int.parse(sharedPreferences.getString(sharedSalesAgentId) ?? "0"),
        "group_id": 0,
        'comment': "",
        'description': "",
        'from_shop': 'true',
        'store_id': int.parse(sharedPreferences.getString(sharedStoreId) ?? "0"),
        'sales_agent_id': isAgent== true ? sharedPreferences.getString(sharedSalesAgentId) ?? "0" : "2",
        'customer_id': int.parse(sharedPreferences.getString(sharedCustomerId) ?? "0"),
        'region_id': 0,
        'dt_kt_data': [],
        'currency_id': int.parse(sharedPreferences.getString(sharedCurrencyId) ?? "0"),
        'currency_value': int.parse(sharedPreferences.getString(sharedCurrencyValue) ?? "0"),
        'price_type_id': int.parse(sharedPreferences.getString(sharedPriceTypeId) ?? "2"),
      };
      ...
      if(isAgent == true){
        url = baseUrl + wholeOrderAgentPHP;
      } else {
        url = baseUrl + wholeOrderPHP;
      }
      ...
      if (response.statusCode == 200) {
        ...
        listError.add(ErrorModel(currencyId: 0,currencyName: "",price: 0,productId: 0,productName: response.body,quantity: 0,residue: 0));
        return listError;
      }
